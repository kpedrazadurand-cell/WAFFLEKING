...
const PACKS = [
  {
    id: "special",
    name: "Waffle Especial (1 piso)",
    price: 25,
    incTop: 3,
    incSir: 2,
    desc: "Incluye 3 toppings + 2 siropes + dedicatoria",
    img: "assets/ref-special.jpg",
  },
  {
    id: "king",
    name: "Waffle King (2 pisos)",
    price: 45,
    incTop: 4,
    incSir: 3,
    desc: "Incluye 4 toppings + 3 siropes + dedicatoria",
    img: "assets/ref-king.jpg",
  },
];
...
function useCartCount() {
  const getCount = () =>
    JSON.parse(localStorage.getItem("wk_cart") || "[]").reduce(
      (a, b) => a + (+b.qty || 0),
      0
    );
...
function WelcomeModal({ open, onClose, onStart }) {
  ...
  return (
    <div
      ...
      onClick={(e) => {
        if (e.target === e.currentTarget) closeWithAnim(onClose);
      }}
    ...
    >
      <div
        ...
        style={{
          ...
          boxShadow:
            "0 20px 50px rgba(58,17,4,.28), 0 4px 18px rgba(58,17,4,.15)",
          transform: visible
            ? "scale(1) translateY(0)"
            : "scale(.98) translateY(6px)",
          opacity: visible ? 1 : 0,
        ...
        <div className="grid md:grid-cols-[240px,1fr] gap-4 p-4 md:p-5 items-center">
          ...
          <div className="flex flex-col items-center md:items-start justify-center gap-3 md:gap-3.5 md:pl-5 md:border-l md:border-amber-200">
            <h2
              ...
              style={{
                backgroundImage:
                  "linear-gradient(90deg,#b32b11 0%, #6c1e00 100%)",
            ...
            <div
              ...
              style={{
                background: "linear-gradient(90deg,#c28432,#b32b11)",
              }}
            />
            <p className="text-sm md:text-[15px] text-[#4e3427] leading-relaxed text-center md:text-left">
              Aquí horneamos felicidad capa por capa. ¿List@ para crear tu
              waffle perfecto?
            </p>
            <button
              ...
              style={{
                background:
                  "linear-gradient(180deg,#3a1104,#2a0c02)",
...
function ReminderModal({ open, onClose, cartCount }) {
  ...
  const close = () => {
    ...
    setTimeout(() => {
      if (never) {
        try {
          localStorage.setItem("wk_reminder_optout", "1");
        } catch (e) {}
      }
      onClose();
    }, 200);
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="wk-remind-title"
      onClick={(e) => {
    ...
    >
      ...
      <div
        className="relative bg-white rounded-2xl border-2 w-full max-w-[680px] overflow-hidden"
        style={{
          borderColor: "#c28432",
          boxShadow:
            "0 16px 40px rgba(58,17,4,.26), 0 4px 18px rgba(58,17,4,.15)",
          transform: visible
            ? "scale(1) translateY(0)"
            : "scale(.98) translateY(6px)",
          opacity: visible ? 1 : 0,
      ...
      >
        ...
        <button
          ...
          style={{
            ...
            background: "#fff",
            color: "#3a1104",
            boxShadow: "0 6px 16px rgba(58,17,4,.18)",
          }}
        >
          ×
        </button>
        ...
        <div className="grid md:grid-cols-[1fr,260px] gap-0">
          ...
          <div className="p-4 md:p-5 order-1 md:order-none">
            ...
            <div
              className="mx-auto md:mx-0 my-2 h-[3px] w-14 rounded-full"
              style={{
                background: "linear-gradient(90deg,#c28432,#b32b11)",
              }}
            />
            ...
            <p className="text-[14px] md:text-[15px] text-[#4e3427] leading-relaxed text-center md:text-left">
              Tienes{" "}
              <b style={{ color: "#8e240c" }}>{cartCount}</b> waffle
              {cartCount > 1 ? "s" : ""} en tu carrito. ¿Deseas retomarlo?
            </p>
          ...
          <div className="order-2 md:order-none px-4 pb-4 md:px-5 md:py-5">
            <div
              className="rounded-[14px] border-2 overflow-hidden mx-auto"
              style={{
                borderColor: "#c28432",
    ...
    <header className="sticky top-0 z-40 glass border-b border-amber-100/70">
      <div className="max-w-5xl mx-auto px-4 pt-3 pb-2">
        <div className="flex items-center gap-3">
          <img
            src={LOGO}
            className="h-10 w-10 rounded-xl ring-1 ring-amber-200 object-contain"
            alt="Waffle King"
          />
          <div className="leading-4">
            <h1 className="font-extrabold text-lg">Waffle King</h1>
            <p className="text-xs text-slate-700">
              Pedidos online — Lima Norte
            </p>
          </div>
          <button
            onClick={() => (location.href = "checkout.html")}
            className="ml-auto relative rounded-full border border-amber-300 p-2 hover:bg-amber-50"
          ...
          >
            ...
            >
              <path
                fill="currentColor"
                d="M7 4h-2l-1 2h2l3.6 7.59l-1.35 2.45A1.99 1.99 0 0 0 10 19h9v-2h-9l1.1-2h7.45a2 2 0 0 0 1.79-1.11l3.58-6.49A1 1 0 0 0 23 5H6.21l-.94-2ZM7 20a2 2 0 1 0 4 0a2 2 0 0 0-4 0m8 0a 2 2 0 1 0 4 0a2 2 0 0 0-4 0"
              />
            </svg>
            {count > 0 && (
              <span className="absolute -top-1 -right-1 bg-[#3a1104] text-white text-xs px-1.5 py-0.5 rounded-full">
                {count}
              </span>
            )}
          </button>
    ...
    >
      ...
      >
        <div className="px-5 py-3 border-b flex items-center justify-between">
          ...
          <button
            className="btn-pill border focus:outline-none"
            onClick={onClose}
          >
            Cerrar
          </button>
        ...
        <img
          ...
          onError={(e) => {
            ...
            toast("No se encontró la imagen");
          }}
        />
      </div>
    </div>
...
function App() {
  useEffect(() => {
    try {
      if (localStorage.getItem("wk_clear_delivery") === "1") {
  ...
  const [prem, setPrem] = useState(
    Object.fromEntries(PREMIUM.map((p) => [p.id, 0]))
  );
  ...
  const sirsExtra = locked
    ? 0
    : sirs.reduce(
        (a, id) => a + (SIROPES.find((s) => s.id === id)?.extra || 0),
        0
      );
  const premCost = locked
    ? 0
    : Object.entries(prem).reduce(
        (a, [id, q]) =>
          a + (PREMIUM.find((p) => p.id === id)?.price || 0) * (+q || 0),
        0
      );
  const masaDelta = masaId
    ? MASAS.find((m) => m.id === masaId)?.delta || 0
    : 0;
  const unit = locked
    ? 0
    : (pack?.price || 0) + masaDelta + sirsExtra + premCost;
  const total = locked ? 0 : unit * qty;

  function requirePack() {
    if (locked) {
      toast("Debes seleccionar un waffle para continuar");
      return true;
    }
    return false;
  }
  ...
  function add() {
    if (requirePack()) return;
    if (!masaId) {
      toast("Selecciona el tipo de masa");
      return;
    }
    if (qty < 1) {
    ...
    const item = {
      ...
      masaId,
      masaName:
        MASAS.find((m) => m.id === masaId)?.name ||
        "Clásica (harina de trigo)",
      masaDelta,
      toppings: TOPS.filter((t) => tops.includes(t.id)).map(
        (t) => t.name
      ),
      siropes: SIROPES.filter((s) => sirs.includes(s.id)).map((s) => ({
        name: s.name,
        extra: s.extra || 0,
      })),
      premium: PREMIUM.filter((p) => (+prem[p.id] || 0) > 0).map(
        (p) => ({
          name: p.name,
          price: p.price,
          qty: +prem[p.id],
        ...
        <Block title="Elige tu waffle">
              ...
              >
                <div className="flex items-start justify-between">
                  <div>
                    <h4
                      className={
                        p.id === packId ? "font-bold" : "font-medium"
                      }
                    >
                      {p.name}
                    </h4>
                    ...
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        setPreview({ src: p.img, title: p.name });
                      }}
                      className={
                        "mt-2 inline-flex items-center gap-1 text-xs text-amber-800 underline underline-offset-2 decoration-amber-300 hover:decoration-amber-600 " +
                        FOCUS_OFF
                      }
                    ...
                    >
                      ...
                      >
                        <path
                          fill="currentColor"
                          d="M21 19V5H3v14h18ZM21 3a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h18ZM8 11l2.03 2.71l2.72-3.62L16 14h-8Z"
                        />
                      </svg>
          ...
          {!pack && (
            <div className="mt-2 text-xs text-slate-600">
              Selecciona un waffle para desbloquear los siguientes
              pasos.
            </div>
          )}
        </Block>
            ...
            {MASAS.map((m) => {
              ...
              return (
                <button
                  ...
                  title={
                    locked
                      ? "Debes seleccionar un waffle para continuar"
                      : ""
                  }
                >
        ...
        >
          ...
          >
            {TOPS.map((t) => {
              ...
              const dis =
                !active && tops.length >= (pack?.incTop || 0);
              return (
                <button
                  key={t.id}
                  onClick={() =>
                    toggle(tops, setTops, pack?.incTop || 0, t.id)
                  }
                  className={
                    "text-left rounded-xl border px-3 py-2 " +
                    FOCUS_OFF +
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    {active && (
                      <span className="text-xs text-[#3a1104]">
                        ✓
                      </span>
                    )}
                  </div>
        ...
        >
          ...
          >
            {SIROPES.map((s) => {
              ...
              const dis =
                !active && sirs.length >= (pack?.incSir || 0);
              return (
                <button
                  key={s.id}
                  onClick={() =>
                    toggle(sirs, setSirs, pack?.incSir || 0, s.id)
                  }
                  className={
                    "text-left rounded-xl border px-3 py-2 " +
                    FOCUS_OFF +
          ...
          <p className="text-xs text-slate-600 mt-2">
            * Hersheys agrega S/ 2.00 al total aunque esté dentro del
            pack.
          </p>
        </Block>
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      className={"px-2 py-1 rounded-full border " + FOCUS_OFF}
                      onClick={() => setPremium(p.id, -1)}
                    >
                      −
                    </button>
                    <span className="w-8 text-center">
                      {locked ? 0 : prem[p.id] || 0}
                    </span>
                    <button
                      className={"px-2 py-1 rounded-full border " + FOCUS_OFF}
                      onClick={() => setPremium(p.id, 1)}
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            <div>
              ...
              <input
                value={rec}
                onChange={(e) =>
                  setRec(e.target.value.slice(0, 60))
                }
                className="mt-1 w-full rounded-lg border border-slate-300 p-2 focus:outline-none"
                placeholder="Ej: Mackey"
              />
            </div>
            <div className="sm:col-span-2">
              ...
              <textarea
                value={notes}
                onChange={(e) =>
                  setNotes(e.target.value.slice(0, 180))
                }
                className="mt-1 w-full rounded-lg border border-slate-300 p-3 focus:outline-none"
                rows="3"
                placeholder="Ej: Para Mackey con mucho amor. ¡Feliz cumple!"
              />
          ...
          <div className="text-sm">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className="text-xs text-slate-600">
                {!pack
                  ? "(selecciona un waffle)"
                  : `(${soles(unit)} c/u)`}
              </span>
            </div>
            <div className="text-xs text-slate-600">
              Base del pack + sirope(s) con extra + premium
              seleccionados.
            </div>
          </div>
          <div className="flex items-center gap-2">
            <button
              className={"px-3 py-2 rounded-full border " + FOCUS_OFF}
            ...
            <button
              ...
              style={
                !pack
                  ? {
                      background:
                        "linear-gradient(180deg, rgba(58,17,4,0.62), rgba(58,17,4,0.46))",
                      opacity: 1,
