...
function useCartCount() {
  const [c, setC] = useState(() =>
    JSON.parse(localStorage.getItem("wk_cart") || "[]").reduce((a, b) => a + b.qty, 0)
  );
...
function App() {
  ...
  function add() {
    ...
    const item = {
      ...
      incSir: pack.incSir,
      masaId,
      masaName: MASAS.find((m) => m.id === masaId)?.name || "Clásica (harina de trigo)",
      masaDelta,
        ...
        <div className="rounded-2xl bg-white border border-slate-200 p-5 shadow-soft flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              disabled={!pack}
              className={
                "btn-pill text-white " + FOCUS_OFF + " " + (!pack ? "btn-disabled" : "hover:bg-[#2a0c02]")
              }
              style={
                !pack
                  ? {
