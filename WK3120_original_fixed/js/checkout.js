    ...
    >
      ...
      >
        <div className="px-5 py-3 border-b flex items-center justify-between">
          ...
          <button className="btn-pill border focus:outline-none" onClick={onClose}>
            Cerrar
          </button>
        </div>
        <img
...
function App() {
  ...
  const sirsExtra = locked
    ? 0
    : sirs.reduce((a, id) => a + (SIROPES.find((s) => s.id === id)?.extra || 0), 0);
  const premCost = locked
    ? 0
    : Object.entries(prem).reduce(
        (a, [id, q]) => a + (PREMIUM.find((p) => p.id === id)?.price || 0) * (+q || 0),
        0
      );
  ...
  function setPremium(id, d) {
    if (requirePack()) return;
    setPrem((prev) => ({ ...prev, [id]: Math.max(0, (+prev[id] || 0) + d) }));
  }
  ...
  function add() {
    ...
    const item = {
      ...
      masaId,
      masaName:
        MASAS.find((m) => m.id === masaId)?.name || "Clásica (harina de trigo)",
      masaDelta,
      ...
      premium: PREMIUM.filter((p) => (+prem[p.id] || 0) > 0).map((p) => ({
        name: p.name,
        price: p.price,
        qty: +prem[p.id],
      })),
    ...
    <div>
      ...
      <main className="max-w-5xl mx-auto px-4 py-6 space-y-6">
        ...
        <Block title="Elige tu waffle">
            ...
            {PACKS.map((p) => (
              ...
              >
                <div className="flex items-start justify-between">
                  <div>
                    <h4 className={p.id === packId ? "font-bold" : "font-medium"}>
                      {p.name}
                    </h4>
          ...
          {!pack && (
            <div className="mt-2 text-xs text-slate-600">
              Selecciona un waffle para desbloquear los siguientes pasos.
            </div>
          )}
        </Block>
            ...
            {MASAS.map((m) => {
              ...
              return (
                <button
                  ...
                  title={
                    locked ? "Debes seleccionar un waffle para continuar" : ""
                  }
                >
                  <div className="flex items-center justify-between">
                    ...
                    {m.delta > 0 && (
                      <span className="text-xs">+{soles(m.delta)}</span>
                    )}
                  </div>
                </button>
              );
        ...
        <Block
          ...
          extra={<Pill used={tops.length} total={pack?.incTop} label="Toppings" />}
        >
          <div
            className={
              "grid sm:grid-cols-2 gap-2 " +
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    {active && (
                      <span className="text-xs text-[#3a1104]">✓</span>
                    )}
                  </div>
                </button>
              );
        ...
        <Block
          ...
          extra={<Pill used={sirs.length} total={pack?.incSir} label="Siropes" />}
        >
          <div
            className={
              "grid sm:grid-cols-2 gap-2 " +
          ...
          <p className="text-xs text-slate-600 mt-2">
            * Hersheys agrega S/ 2.00 al total aunque esté dentro del pack.
          </p>
        </Block>
        ...
        <Block title="Dedicatoria y destinatario">
          <div className="grid sm:grid-cols-2 gap-3">
            <div>
              ...
              <input
                value={rec}
                onChange={(e) => setRec(e.target.value.slice(0, 60))}
        ...
        <div className="rounded-2xl bg-white border border-slate-200 p-5 shadow-soft flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div className="text-sm">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className="text-xs text-slate-600">
                {!pack ? "(selecciona un waffle)" : "(" + soles(unit) + " c/u)"}
              </span>
            </div>
            <div className="text-xs text-slate-600">
              Base del pack + sirope(s) con extra + premium seleccionados.
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              disabled={!pack}
              title={!pack ? "Debes seleccionar un waffle para continuar" : ""}
            >
              −
            </button>
